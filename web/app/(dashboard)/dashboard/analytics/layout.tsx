...
export default function AnalyticsLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <PageContainer className="space-y-6">
      ...
      />
      <nav aria-label="Analytics navigation" className="flex gap-1 overflow-x-auto border-b border-border pb-2">
        {items.map(({ label, href, icon: Icon }) => {
          const tab = new URLSearchParams(href.split("?")[1]).get("tab") ?? "source";
