...
export function getEmailChangeVerificationTemplate(name: string | null, verificationUrl: string): string {
  ...
  const content = `
    ...
    <p>Hi ${displayName},</p>
    <p>We received a request to change the email address associated with your Getlist account to this one. Please confirm this change by clicking the button below.</p>
  ...
  `;
  return baseLayout(content, 'Verify your new Getlist email address');
}
