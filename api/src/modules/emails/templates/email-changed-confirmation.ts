...
export function getEmailChangedConfirmationTemplate(name: string | null, newEmail: string): string {
  ...
  const content = `
    ...
    <p>Hi ${displayName},</p>
    <p>This is a confirmation that the email address for your Getlist account has been successfully changed to <strong>${newEmail}</strong>.</p>
  ...
  `;
  return baseLayout(content, 'Your Getlist email address has been changed');
}
