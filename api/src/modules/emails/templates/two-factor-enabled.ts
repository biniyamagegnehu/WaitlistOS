...
export function getTwoFactorEnabledTemplate(name: string | null): string {
  ...
  const content = `
    ...
    <p>Hi ${displayName},</p>
    <p>This is a confirmation that Two-Factor Authentication (2FA) has been successfully enabled for your Getlist account.</p>
  ...
  `;
  return baseLayout(content, 'Two-Factor Authentication enabled on Getlist');
}
